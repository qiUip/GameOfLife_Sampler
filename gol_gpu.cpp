...
template<typename CellT, typename HostGridT>
size_t GPUEngine<CellT, HostGridT>::getStride() const { return stride_; }

template<typename CellT, typename HostGridT>
void GPUEngine<CellT, HostGridT>::commitBoundaries() {
  ops_.copyH2D(d_current_,
...
template<typename CellT, typename HostGridT>
void GPUEngine<CellT, HostGridT>::writeToFile(const std::string &filename) const {
  ops_.copyD2H(hostGrid_.getData(), d_current_,
               rows_ * stride_ * sizeof(CellT));
