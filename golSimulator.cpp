...
static std::unique_ptr<GameOfLife> setupGame(GridType &fullGrid, int mpiRank,
                                             int mpiSize) {
    GridType localGrid;
    if (mpiSize > 1) {
        if (mpiRank == 0)
...
int main(int argc, char **argv) {
    ...
    switch (params.engine) {
...
#endif
        {
            ...
            if (mpiRank == 0) {
                if (params.randomInit) {
                    ...
                    bg = BitGrid(params.fullGridRows, params.fullGridColumns,
                                 params.alive, rng);
                } else {
                    bg = BitGrid(grid);
                }
        ...
        default:
            if (mpiRank == 0)
                std::cerr
                    << "Error: engine not available (built without CUDA?)\n";
            MPI_Finalize();
            return EXIT_FAILURE;
    }
