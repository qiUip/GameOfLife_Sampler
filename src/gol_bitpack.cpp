...
void BitPackGameOfLife::takeStep() {
    ...
    const uint64_t lastMask =
        (cols_ % 64 == 0) ? ~uint64_t(0) : (uint64_t(1) << (cols_ % 64)) - 1;

    uint64_t *zeroRow =
        static_cast<uint64_t *>(std::calloc(nw, sizeof(uint64_t)));
...
#pragma omp parallel for schedule(static)
    for (size_t row = 0; row < rows_; ++row) {
        ...
        for (size_t w = 0; w < nw; ++w) {
            ...
            const uint64_t cR =
                (cC >> 1) | (w < nw - 1 ? c[w + 1] << 63 : 0ULL);
            uint64_t pC = p[w],
                     pL = (pC << 1) | (w > 0 ? p[w - 1] >> 63 : 0ULL),
                     pR = (pC >> 1) | (w < nw - 1 ? p[w + 1] << 63 : 0ULL);
            uint64_t nC = n[w],
                     nL = (nC << 1) | (w > 0 ? n[w - 1] >> 63 : 0ULL),
...
size_t BitPackGameOfLife::getStride() const {
    return wordsPerRow_;
}

CellKind BitPackGameOfLife::getCellKind() const {
